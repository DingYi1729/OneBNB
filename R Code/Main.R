...
## 1BNB
Core_Yhat = read.csv("Data/CollaBoots_30core_30p.csv",header = FALSE)
## !Warning: this process might take a while.
## 2(2) Obtain the 1-BNB-estimated CI for degree distribution of 30-core
...
## Sample 
vid_sample = Node_Sample(g,100/length(V(g)),1)
...
Degree_Plot_sample = 
  ggplot(Deg_dist_sample, aes(x = n, y = deg_dist_sample)) +
  geom_line() +
  geom_point(size = 1) +
  theme_bw() +
  scale_x_continuous(breaks = seq(0,250,50)) +
...
## 1-BNB
Sample_Yhat = readMat("Data/BK_sample_17p_B200.mat")
...
Degree_Plot_adj             = Degree_Plot + ylim(c(0,0.6))
Degree_CI_Plot_1BNB_adj     = Degree_CI_Plot_1BNB + ylim(c(0,0.6))
Degree_CI_Plot_CVE_adj      = Degree_CI_Plot_CVE + ylim(c(0,0.6))
Degree_CI_Plot_MRME_adj     = Degree_CI_Plot_MRME + ylim(c(0,0.6))
Degree_CI_Plot_CHTE_adj     = Degree_CI_Plot_CHTE + ylim(c(0,0.6))
Degree_CI_Plot_SnowBoot_adj = Degree_CI_Plot_SnowBoot + ylim(c(0,0.6))
